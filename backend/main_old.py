...
async def get_time_based_statistics(
    ...
    print(f"[DEBUG] time-based API 호출됨 - period: {period}, start_date: {start_date}, end_date: {end_date}")
    try:
        # period 검증
        if period not in ['daily', 'weekly', 'monthly', 'custom']:
            raise HTTPException(status_code=400, detail="period는 'daily', 'weekly', 'monthly', 'custom' 중 하나여야 합니다.")
        ...
        try:
            with connection.cursor() as cursor:
                if period == 'custom' and start_date and end_date:
                    # 사용자 정의 날짜 범위 처리
                    print(f"[DEBUG] 사용자 정의 날짜 범위 조회: {start_date} ~ {end_date}")
                    ...
                    cursor.execute("""
                        ...
                        FROM dates d
                        LEFT JOIN ui_log_page_visits pv ON DATE(pv.created_at) = d.date
                        GROUP BY d.date
                        ORDER BY d.date
                    """, (start_date, end_date))
        ...
        print(f"[ERROR] 시간별 통계 조회 실패: {str(e)}")
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if period == 'custom' and start_date and end_date:
                ...
                date_condition = "AND created_at >= DATE_SUB(NOW(), INTERVAL 14 DAY)"
            elif period == 'weekly':
                date_condition = "AND created_at >= DATE_SUB(NOW(), INTERVAL 12 WEEK)"
            elif period == 'monthly':
                date_condition = "AND created_at >= DATE_SUB(NOW(), INTERVAL 12 MONTH)"
            ...
            if period == 'daily' or period == 'custom':
                ...
                date_format = "CONCAT(YEAR(created_at), '년 ', LPAD(MONTH(created_at), 2, '0'), '월 ', (WEEK(created_at, 1) - WEEK(DATE_SUB(created_at, INTERVAL DAY(created_at)-1 DAY), 1) + 1), '주차')"
            elif period == 'monthly':
                # 월별 집계
                group_by = "DATE_FORMAT(created_at, '%Y-%m')"
                date_format = "DATE_FORMAT(created_at, '%Y년 %m월')"
            else:
                # 기본값: 일별
                group_by = "DATE(created_at)"
                date_format = "DATE(created_at)"
            ...
            if period == 'daily' or period == 'custom':
                ...
                query = f"""
                    SELECT 
                        CONCAT(YEAR(created_at), '년 ', LPAD(MONTH(created_at), 2, '0'), '월 ', (WEEK(created_at, 1) - WEEK(DATE_SUB(created_at, INTERVAL DAY(created_at)-1 DAY), 1) + 1), '주차') as period_label,
                ...
                query = f"""
                    SELECT 
                        DATE_FORMAT(created_at, '%%Y년 %%m월') as period_label,
            ...
            print(f"[DEBUG] TOS Executing query: {query}")
            print(f"[DEBUG] TOS With params: {params}")
            ...
            print(f"[DEBUG] Total TOS inspection records: {count_result[0] if count_result else 0}")
        ...
        print(f"[ERROR] TOS 검사 히스토리 조회 실패: {str(e)}")
        import traceback
        traceback.print_exc()
...
async def get_ais_inspection_history(
    ...
    try:
        print(f"[DEBUG] AIS inspection history called with period={period}, start_date={start_date}, end_date={end_date}")
        ...
        with connection.cursor() as cursor:
            ...
            """)
            count_result = cursor.fetchone()
            print(f"[DEBUG] Total AIS inspection records: {count_result[0] if count_result else 0}")
        ...
        print(f"[ERROR] AIS 검사 히스토리 조회 실패: {str(e)}")
        import traceback
        traceback.print_exc()
