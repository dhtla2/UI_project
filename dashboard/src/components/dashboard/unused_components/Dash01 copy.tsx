...
const Dash01: React.FC = () => {
  ...
  const validateTimeStats = (data: any): TimeBasedStatistics | null => {
    ...
    return {
      ...
      api_calls: Array.isArray(data.api_calls) ? data.api_calls : []
    };
  };
  ...
  const fetchTimeBasedStatistics = async () => {
    try {
      ...
      const response = await fetch(`/ui/statistics/time-based?period=${timePeriod}`);
  ...
  const fetchVisitorTrends = async () => {
    try {
      ...
      console.log('방문자 트렌드 데이터 로드 실패:', err);
      // 기본값 설정
      setVisitorTrends({
