...
import { 
  ...
  fetchPortVsslInspectionHistory,
  fetchPortVsslLatestResults,
  fetchPortVsslQualityStatus,
...
interface PortMisVsslNoPageProps {
...
const PortMisVsslNoPage: React.FC<PortMisVsslNoPageProps> = ({ currentPage, onPageChange }) => {
  // 상태 관리
  ...
  const [portVsslData, setPortVsslData] = useState<any>(null);
  const [historyData, setHistoryData] = useState<any>(null);
  const [portVsslQualityStatus, setPortVsslQualityStatus] = useState<any>(null);
  ...
  const loadInspectionHistory = async (period: 'daily' | 'weekly' | 'monthly' | 'custom' = 'daily', startDate?: string, endDate?: string) => {
    try {
      const historyData = await fetchPortVsslInspectionHistory(period, startDate, endDate);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // PortMisVsslNo 데이터, API 품질 데이터, 검사 히스토리, 최신 검사 결과를 병렬로 로드
        console.log('🚀 Starting parallel API calls for PortMisVsslNo...');
        const results = await Promise.allSettled([
          fetchPortVsslSummary().then(result => {
            console.log('✅ PortMisVsslNo Summary loaded:', result);
            return result;
          }),
          ...
          fetchPortVsslInspectionHistory('daily').then(result => {
            console.log('✅ Inspection History loaded:', result?.length, 'items');
            return result;
          }),
          fetchPortVsslLatestResults().then(result => {
        ...
        // 각 결과를 안전하게 처리
        const portVsslDataResult = results[0].status === 'fulfilled' ? results[0].value : null;
        ...
        setPortVsslData(portVsslDataResult);
        setApiQualityData(apiData);
        setInspectionHistory(historyData);
        setLatestInspectionResults(latestResults);
        
        // PortMisVsslNo 품질 상태 데이터도 별도로 로드
        try {
          const qualityStatus = await fetchPortVsslQualityStatus();
          setPortVsslQualityStatus(qualityStatus);
        } catch (err) {
          console.warn('PortMisVsslNo 품질 상태 데이터 로드 실패:', err);
        ...
        // 에러 발생 시 빈 배열로 설정
        setPortVsslData(null);
        setApiQualityData([]);
        setInspectionHistory([]);
        setPortVsslQualityStatus(null);
        ...
        <div className="dashboard-main-content">
          <div className="dashboard-top-tabs">
            ...
            <CommonTopSection 
              ...
              matchData={{ matchQualitySummary: portVsslData }}
            />
...
export default PortMisVsslNoPage;
